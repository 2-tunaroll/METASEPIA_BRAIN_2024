...
const externalPosition = computed(() => {
  if (props.externalX === undefined || props.externalY === undefined) return null

  const relativeX = (props.externalX * props.size) / 2
  const relativeY = (-props.externalY * props.size) / 2
  ...
  return {
    ...
    axisX: props.size / 2 + relativeX,
    axisY: props.size / 2 + relativeY
  }
})
...
const stickStyle = computed(() => {
  ...
  if (externalPosition.value) {
    stickStyle = {
      ...stickStyle,
      position: 'absolute',
      transform: `translate3d(${externalPosition.value.relativeX}px, ${externalPosition.value.relativeY}px, 0)`,
    }
  } 
...
const _pointerDown = (e: PointerEvent) => {
  ...
  if (props.externalX !== undefined || props.externalY !== undefined) return
...
watch([() => props.externalX, () => props.externalY], ([newX, newY]) => {
  if (newX !== undefined && newY !== undefined && !state.dragging) {
    const position = externalPosition.value
    if (position) {
      _updatePos(position)
    }
  }
